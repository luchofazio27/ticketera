...
require_once("../models/Subcategoria.php"); //Conexion con el modelo Subcategoria
$subcategoria = new Subcategoria(); // Declaramos esa clase
...
switch ($_GET["op"]) { //$_GET es una matriz de variables que se pasan al script actual a través de los parámetros de URL
    case "guardaryeditar":
            ...
            if(count($datos)==0){
                if(empty($_POST["cats_id"])){
                    $subcategoria->insert_subcategoria($_POST["cat_id"],$_POST["cats_nom"], $_POST["sec_id"]);
                    echo "1";
                }else {
                    $subcategoria->update_subcategoria($_POST["cats_id"],$_POST["cat_id"],$_POST["cats_nom"], $_POST["sec_id"]);
                    echo "2";
                }
            }else{
                echo "0";
    ...
    case "mostrar";
        ...
        break;
    case "combo":
        $datos = $subcategoria->get_subcategoria($_POST["cat_id"]); //Declaramos la variable datos para almacenar la consulta // el símbolo -> es un operador que se usa para acceder a las propiedades y métodos de un objeto
        $html = "";
        if (is_array($datos) == true and count($datos) > 0) { //si es un array y si tiene datos
            ...
            foreach ($datos as $row) { // $row es una matriz que puede contener varios valores al mismo tiempo
                $html .= "<option value='" . $row['cats_id'] . "'>" . $row['cats_nom'] . "</option>"; //declaramos un html con dichos valores
            }
